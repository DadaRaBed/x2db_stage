...
function showChangePasswordModal() {
  ...
  overlay.innerHTML = `
    <div style="background: #ffffff; border-radius: 12px; padding: 0; max-width: 480px; width: 95%; box-shadow: 0 20px 60px rgba(0, 0, 0, 0.3); overflow: hidden;">
      ...
      <div style="padding: 24px;">
        <div class="form-group" style="margin-bottom: 1rem;">
          <label style="display: block; font-weight: 600; margin-bottom: 6px; color: #334155; font-size: 0.9rem;">Ancien mot de passe :</label>
          <input type="password" id="chpwd-old" placeholder="Votre mot de passe actuel" autocomplete="current-password"
            style="width: 100%; padding: 0.6rem; border: 1px solid #cbd5e1; border-radius: 6px; font-size: 0.9rem; box-sizing: border-box;" />
        </div>
        <div class="form-group" style="margin-bottom: 1rem;">
          <label style="display: block; font-weight: 600; margin-bottom: 6px; color: #334155; font-size: 0.9rem;">Nouveau mot de passe :</label>
          <input type="password" id="chpwd-new" placeholder="Au moins 6 caracteres" autocomplete="new-password"
            style="width: 100%; padding: 0.6rem; border: 1px solid #cbd5e1; border-radius: 6px; font-size: 0.9rem; box-sizing: border-box;" />
        </div>
        <div class="form-group" style="margin-bottom: 1rem;">
          <label style="display: block; font-weight: 600; margin-bottom: 6px; color: #334155; font-size: 0.9rem;">Confirmer :</label>
  ...
  overlay.querySelector("#btn-save-chpwd").onclick = async () => {
    ...
    if (!oldPwd || !newPwd || !confirm) {
      statusEl.innerHTML =
        '<p style="color: #dc3545; font-size: 0.9rem;">Tous les champs sont obligatoires.</p>';
      return;
    }
    if (newPwd.length < 6) {
      statusEl.innerHTML =
        '<p style="color: #dc3545; font-size: 0.9rem;">Minimum 6 caracteres.</p>';
      return;
    }
    if (newPwd !== confirm) {
      statusEl.innerHTML =
        '<p style="color: #dc3545; font-size: 0.9rem;">Les mots de passe ne correspondent pas.</p>';
      return;
    }

    statusEl.innerHTML =
      '<p style="color: #1a4d3a; font-size: 0.9rem;">Modification...</p>';
    ...
    try {
      ...
      if (res && res.success) {
        statusEl.innerHTML =
          '<p style="color: #27ae60; font-size: 0.9rem;">Mot de passe modifie.</p>';
        setTimeout(() => {
          close();
        ...
        statusEl.innerHTML = `<p style="color: #dc3545; font-size: 0.9rem;">${escapeHtml(res?.message || "Erreur.")}</p>`;
      }
    } catch (err) {
      statusEl.innerHTML = `<p style="color: #dc3545; font-size: 0.9rem;">${escapeHtml(err.message)}</p>`;
    }
  };
}
