// ============================================
// MODAL PLEIN ECRAN (avec theme sombre)
// ============================================
...
function openFullScreenModal(titleText, htmlContent) {
  ...
  overlay.style.cssText = `
    ...
    z-index: 99999; display: flex; justify-content: center; align-items: center; padding: 0;
  `;

  overlay.innerHTML = `
    <div style="background: var(--card-bg, #ffffff); color: var(--text-color, #1a1a2e); width: 100vw; height: 100vh; border-radius: 0; display: flex; flex-direction: column; overflow: hidden;">
      <div style="padding: 16px 24px; border-bottom: 1px solid var(--border-color, #e2e8f0); display: flex; justify-content: space-between; align-items: center; background: var(--bg-secondary, #f8fafc);">
        <h2 style="margin: 0; font-size: 1.4rem; display: flex; align-items: center; gap: 12px; color: var(--text-color, #1a1a2e);">
          <i class="fas fa-expand" style="color: #1a4d3a;"></i>
          <span id="modal-title-text">${escapeHtml(titleText)}</span>
        </h2>
        <button type="button" id="close-fullscreen-modal" style="background: none; border: none; font-size: 2rem; cursor: pointer; color: var(--text-color, #1a1a2e);">&times;</button>
      </div>
      <div id="fullscreen-modal-body" style="padding: 24px; overflow-y: auto; flex: 1; background: var(--card-bg, #ffffff); color: var(--text-color, #1a1a2e);"></div>
      <div style="padding: 14px 24px; border-top: 1px solid var(--border-color, #e2e8f0); background: var(--bg-secondary, #f8fafc); display: flex; justify-content: flex-end; gap: 10px; flex-wrap: wrap;">
  ...
  style.textContent = `
    #fullscreen-modal-body table {
      background: var(--card-bg, #ffffff) !important;
      color: var(--text-color, #1a1a2e) !important;
    }
    #fullscreen-modal-body th {
    ...
    #fullscreen-modal-body tr:nth-child(even) td {
      background: var(--bg-secondary, #f8fafc) !important;
    }
    #fullscreen-modal-body tr:hover td {
      background: rgba(26, 77, 58, 0.12) !important;
    }
